...
def SvrtiDesno(x, y, side, obstacle_list, dots):
...
def SvrtiLevo(x, y, side, obstacle_list, dots):
    dots_list_new = list(dots)
...
def ProdolzhiPravo(x, y, side, obstacle_list, dots):
    dots_list_new = list(dots)
    if side == 'istok':
        new_side = 'istok'
        if y < 9 and [x, y + 1] not in obstacle_list:
...
def ProdolzhiNazad(x, y, side, obstacle_list, dots):
    dots_list_new = list(dots)
    if side == 'istok':
        new_side = 'zapad'
        if y > 0 and [x, y - 1] not in obstacle_list:
...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        if x_new != x or y_new != y:
            successors['SvrtiLevo'] = (x_new, y_new, side_new, tuple(dots_tuple))

        x_new, y_new, side_new, dots_tuple = SvrtiDesno(x, y, side, self.obstacle_list, dots)
        if x_new != x or y_new != y:
            successors['SvrtiDesno'] = (x_new, y_new, side_new, tuple(dots_tuple))
    ...
    def h(self, node):
        ...
        goalList = node.state[3][0]
        x, y = node.state[0], node.state[1]

        # print(goalList)
        # print(x,y)

        return abs(x - goalList[0]) + abs(y - goalList[1])
