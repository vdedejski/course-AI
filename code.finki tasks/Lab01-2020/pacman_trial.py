...
class Pacman(Problem):
    ...
    def successor(self, state):
        ...
        if len(wantedPositions) > 0:
            succesors[state] = wantedPositions
        else:
             succesors[state] = normalPositions
    ...
    def goal_test(self, state):
        for i in range(len(self.game.matrix)):
            for j in range(len(self.game.matrix[i])):
                if self.game.matrix[i][j] == '.': return True
    ...
    def play_game(self):
        print(breadth_first_tree_search(pacman).solution())
