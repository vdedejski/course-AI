...
def DesnoX(x1, y1, x2, y2, x3, y3, obstacles):
    while x1 < 6 and [x1 + 1, y1] not in obstacles and [x1 + 1, y1] != [x2, y2] and [x1 + 1, y1] != [x3, y3]:
        x1 += 1
    return x1


def LevoX(x1, y1, x2, y2, x3, y3, obstacles):
    while x1 > 0 and [x1 - 1, y1] not in obstacles and [x1 - 1, y1] != [x2, y2] and [x1 - 1, y1] != [x3, y3]:
        x1 -= 1
    return x1


def GoreX(x1, y1, x2, y2, x3, y3, obstacles):
    while y1 < 8 and [x1, y1 + 1] not in obstacles and [x1, y1 + 1] != [x2, y2] and [x1, y1 + 1] != [x3, y3]:
        y1 += 1
    return y1


def DoluX(x1, y1, x2, y2, x3, y3, obstacles):
...
class Molecule(Problem):
    ...
    def successor(self, state):
        ...
        # H1
        x_new = DesnoX(h1_x, h1_y, o_x, o_y, h2_x, h2_y, self.obstacles)
        if x_new != h1_x:
            successors['DesnoH1'] = (x_new, h1_y, o_x, o_y, h2_x, h2_y)

        x_new = LevoX(h1_x, h1_y, o_x, o_y, h2_x, h2_y, self.obstacles)
        if x_new != h1_x:
            successors['LevoH1'] = (x_new, h1_y, o_x, o_y, h2_x, h2_y)

        y_new = GoreX(h1_x, h1_y, o_x, o_y, h2_x, h2_y, self.obstacles)
        if y_new != h1_y:
            successors['GoreH1'] = (h1_x, y_new, o_x, o_y, h2_x, h2_y)

        y_new = DoluX(h1_x, h1_y, o_x, o_y, h2_x, h2_y, self.obstacles)
        if y_new != h1_y:
            successors['DoluH1'] = (h1_x, y_new, o_x, o_y, h2_x, h2_y)

        # H2
        x_new = DesnoX(h2_x, h2_y, o_x, o_y, h1_x, h1_y, self.obstacles)
        if x_new != h2_x:
            successors['DesnoH2'] = (h1_x, h1_y, o_x, o_y, x_new, h2_y)

        x_new = LevoX(h2_x, h2_y, o_x, o_y, h1_x, h1_y, self.obstacles)
        if x_new != h2_x:
            successors['LevoH2'] = (h1_x, h1_y, o_x, o_y, x_new, h2_y)

        y_new = GoreX(h2_x, h2_y, o_x, o_y, h1_x, h1_y, self.obstacles)
        if y_new != h2_y:
            successors['GoreH2'] = (h1_x, h1_y, o_x, o_y, h2_x, y_new)

        y_new = DoluX(h2_x, h2_y, o_x, o_y, h1_x, h1_y, self.obstacles)
        if y_new != h2_y:
            successors['DoluH2'] = (h1_x, h1_y, o_x, o_y, h2_x, y_new)

        # O
        x_new = DesnoX(o_x, o_y, h1_x, h1_y, h2_x, h2_y, self.obstacles)
        if x_new != o_x:
            successors['DesnoO'] = (h1_x, h1_y, x_new, o_y, h2_x, h2_y)

        x_new = LevoX(o_x, o_y, h1_x, h1_y, h2_x, h2_y, self.obstacles)
        if x_new != o_x:
            successors['LevoO'] = (h1_x, h1_y, x_new, o_y, h2_x, h2_y)

        y_new = GoreX(o_x, o_y, h1_x, h1_y, h2_x, h2_y, self.obstacles)
        if y_new != o_y:
            successors['GoreO'] = (h1_x, h1_y, o_x, y_new, h2_x, h2_y)

        y_new = DoluX(o_x, o_y, h1_x, h1_y, h2_x, h2_y, self.obstacles)
        if y_new != o_y:
            successors['DoluO'] = (h1_x, h1_y, o_x, y_new, h2_x, h2_y)
